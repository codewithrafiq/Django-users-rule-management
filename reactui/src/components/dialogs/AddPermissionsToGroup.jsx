...
function SimpleDialog(props) {
  const { onClose, permissions, group_id, open } = props;
  ...
  const onClickAddPermissions = async () => {
    ...
    console.log("====================================");
    await axios({
      url: "http://127.0.0.1:8000/addPermissionsOnGroup/",
  ...
  const columns = [
    ...
    { field: "content_type", headerName: "Content Type", width: 130 },
  ];
...
export default function AddPermissionsToGroup({ group_id }) {
  ...
  const handleClickOpen = async () => {
    setLoaded(false);
    await axios({
      url: "http://127.0.0.1:8000/getPermissionsNotInGroup/",
      //   url: `${process.env.REACT_APP_BASE_URL}/permissionUserDontHave/`,
      method: "POST",
      headers: {
        Authorization: `token ${process.env.REACT_APP_AUTH_TOKEN}`,
